...
import { ClientServiceValidationInput, clientServiceValidationSchema } from '../validations/clientServiceValidations/addClientServiceValidationsschema';
...
export class ClientServicesController {
    ...
    async addClientService(c: Context) {
        try {
          const data = await c.req.json();
          const validatedData: ClientServiceValidationInput = await validate(clientServiceValidationSchema, data);
  ...
  async getAllClientServices(c: Context) {
    try {
  ...
  async getClientService(c: Context) {
    try {
  ...
  async deleteClientService(c: Context) {
    try {
      const id = +c.req.param('id');
      if (isNaN(id)) {
        throw new BadRequestException(CLIENT_SERVICES_MESSAGES.INVALID_ID);
